...
- This rule covers Ctrip/Tongcheng web search, Tencent Docs visual verification, AMap web route rendering, ordinary webpage research and login-dependent travel pages. If the agent in-app Browser has a transient setup/tab/navigation problem, retry lightweight recovery before declaring it unavailable: list/reuse the current tab, open a fresh tab, reload or navigate to `about:blank` then the target URL, wait/reconnect, and retry screenshot/navigation. Make several cheap attempts and record the concrete failure reason; do not jump to Playwright after one transient browser issue. Only use Playwright after the first two surfaces are unavailable, blocked or unsuitable for the task.
