...
- Check the rendered left outline after day renumbering. If a Day is visible in the body but missing from the outline, read the affected blocks with `smartcanvas.read`/`smartcanvas.find`; Tencent Docs may serialize a malformed heading as `<Unsupported type="headerundefined" ... />`. Replace those blocks with standard `<Heading level="2|3">DXX ...</Heading>` and re-check the outline.
...
- Day numbering: when an existing long itinerary changes length, create or update a local ordered day ledger before editing Tencent Docs. Use it to regenerate DXX headings, route image alt/captions, overview day counts, hotel anchors, appointment references, map links and cost formulas in one pass. Do not make ad hoc partial renumbering edits in the live document. The final shareable document should keep stable `D1/D2/...` headings for readability and cross-reference clarity; use ordered lists, formulas, mapping tables or scripts only in the local drafting/renumbering pass, not as visible replacements for Day headings. After writing, run a structural readback that counts all Day headings, checks for missing/duplicate D numbers, and confirms there is no `headerundefined` or other unsupported heading block.
