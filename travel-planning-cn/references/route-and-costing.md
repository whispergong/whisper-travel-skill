...
- If itinerary edits add, remove, merge, or split days, update the day ledger before writing: day number, heading, start/end, route image, hotel anchor, reservation references, cost formulas and every later DXX cross-reference. Prefer one renumbering pass from a local ordered list or mapping table, then read back the affected range to catch duplicate/missing day numbers.
